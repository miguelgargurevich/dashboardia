...
export default function DailyPieChart({ token }: DailyPieChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const rawData = [
          { status: 'Abierto', count: 15, color: '#ef4444' },
          { status: 'En Progreso', count: 23, color: '#f59e0b' },
          { status: 'Pendiente', count: 8, color: '#eab308' },
          { status: 'Resuelto', count: 32, color: '#22c55e' }
        ];
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-80">
        <div className="text-gray-400">Cargando distribución diaria...</div>
      </div>
    );
  }
