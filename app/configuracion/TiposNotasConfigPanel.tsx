...
const TiposNotasConfigPanel: React.FC<TiposNotasConfigPanelProps> = ({ 
  ...
  const [tiposState, setTiposState] = useState<TipoNota[]>(tiposNotasProp || []);
  ...
  // Cargar tipos de notas desde el JSON centralizado al montar
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editando) {
      nuevos = tiposState.map((t) =>
        t.id === editando ? { ...t, nombre: form.nombre, descripcion: form.descripcion, color: form.color } : t
      );
      setEditando(null);
    } else {
      nuevos = [
