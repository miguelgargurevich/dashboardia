...
import { FaPlus, FaEdit, FaTrash, FaFolderOpen, FaSave, FaTimes } from "react-icons/fa";
...
const RecursosConfigPanel: React.FC<RecursosConfigPanelProps> = ({ tiposRecursos: tiposRecursosProp, onChange }) => {
  const [tiposRecursos, setTiposRecursos] = useState<TipoRecurso[]>(tiposRecursosProp || []);
  ...
  const obtenerColorPorTailwind = (tailwindColor: string) => {
    if (!tailwindColor || colores.length === 0) return null;
  ...
  const obtenerHexPorTailwind = (tailwindColor: string) => {
    if (!tailwindColor || colores.length === 0) return '#3B82F6'; // Color azul por defecto
  ...
  const manejarSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editando) {
        ...
        const response = await fetch(`/api/config/tipos-recursos/${editando}`, {
          method: 'PUT',
          headers: {
        ...
        if (response.ok) {
          ...
          const nuevosTipos = tiposRecursos.map(tipo => 
            tipo.id === editando ? tipoActualizado : tipo
          );
          setTiposRecursos(nuevosTipos);
        ...
        if (response.ok) {
          ...
          const nuevosTipos = [...tiposRecursos, nuevoTipo];
          setTiposRecursos(nuevosTipos);
          onChange?.(nuevosTipos);
          setAgregando(false);
        }
      ...
      console.error('Error guardando tipo de recurso:', error);
    } finally {
      setLoading(false);
    }
  };

  const manejarEditar = (tipo: TipoRecurso) => {
    const nuevoFormData = { 
      nombre: tipo.nombre, 
  ...
  const manejarEliminar = async (id: string) => {
    if (!confirm('¿Estás seguro de que quieres eliminar este tipo de recurso?')) return;

    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      
      const response = await fetch(`/api/config/tipos-recursos/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      if (response.ok) {
        const nuevosTipos = tiposRecursos.filter(tipo => tipo.id !== id);
        setTiposRecursos(nuevosTipos);
        onChange?.(nuevosTipos);
      }
    } catch (error) {
    ...
    <div className="bg-secondary rounded-xl shadow-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-bold text-accent">Tipos de Recursos</h2>
        </div>
        <button
      ...
      <div className="space-y-4">
        ...
        {agregando && (
          <form onSubmit={manejarSubmit} className="bg-primary/50 p-4 rounded-lg border border-accent/20">
            <h3 className="text-lg font-semibold text-accent mb-4">Nuevo Tipo de Recurso</h3>
            ...
            {editando === tipo.id ? (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div 
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${tipo.color?.split(' ')[0] || 'bg-gray-500/20'} border`}
        ...
        {tiposRecursos.length === 0 && !agregando && (
          <div className="text-center py-8 text-gray-400">
            <FaFolderOpen className="text-4xl mx-auto mb-4 opacity-50" />
            <p>No hay tipos de recursos configurados</p>
            <p className="text-sm">Agrega el primer tipo para comenzar</p>
          </div>
        )}
      </div>
    </div>
