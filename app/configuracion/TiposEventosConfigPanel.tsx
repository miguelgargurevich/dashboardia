...
const TiposEventosConfigPanel: React.FC<TiposEventosConfigPanelProps> = ({ 
  ...
  // Función para mapear iconos de FontAwesome a React Icons
  const obtenerIconoReact = (icono: string) => {
    ...
    const IconComponent = iconMap[icono] || FaCalendarAlt;
  ...
  const obtenerColorPorTailwind = (tailwindColor: string) => {
    if (colores.length === 0 || !tailwindColor) return null;
  ...
  const obtenerHexPorTailwind = (tailwindColor: string) => {
    if (colores.length === 0 || !tailwindColor) return '#3B82F6'; // Color azul por defecto
  ...
  const manejarSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ nombre: '', descripcion: '', icono: '', color: colores.length > 0 ? colores[0].tailwind : '' });
    } catch (error) {
  ...
  const manejarEditar = (tipo: TipoEvento) => {
    setFormData({ nombre: tipo.nombre, descripcion: tipo.descripcion || '', icono: tipo.icono, color: tipo.color });
    setEditando(tipo.id);
    setAgregando(false);
  };
            ...
            {editando === tipo.id ? (
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div 
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${tipo.color ? tipo.color.split(' ')[0] : 'bg-gray-500'} border`}
                    style={{ color: obtenerHexPorTailwind(tipo.color || '') }}
                  >
                    {obtenerIconoReact(tipo.icono)}
                  </div>
                  <div>
                    <h3 className="font-semibold text-white">{tipo.nombre}</h3>
                    {tipo.descripcion && (
                      <p className="text-sm text-gray-300 mb-1">{tipo.descripcion}</p>
                    )}
