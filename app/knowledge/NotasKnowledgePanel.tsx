  ...
  if (notasConfig.loading || temasConfig.loading) {
    return (
      <div className="p-8 text-center">
        <div className="text-lg text-gray-400">Cargando configuración de notas...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-secondary rounded-lg p-6">
        <div className="text-center text-gray-400">
          ...
          <p className="text-lg mb-2">Panel de Notas en Desarrollo</p>
          <p className="text-sm">Sección activa: {seccionActiva}</p>
          <p className="text-sm">Total de tipos configurados: {notasConfig.items.length}</p>
        </div>
      </div>
    </div>
  );
